...
@include('crud::fields.inc.wrapper_start')
    ...
    <select
        name="{{ $field['name'] }}"
        style="width: 100%"
        data-init-function="bpFieldInitLandProfileSelectionInputElement"
...
@push('crud_fields_scripts')
@loadOnce('bpFieldInitLandProfileSelectionInputElement')
<script>
    function bpFieldInitLandProfileSelectionInputElement(element) {
        ...
        function formatState(state) {
            ...
            if($(stateText).find('.land_reference_id').length > 0 ){
                return $(stateText).find('.land_reference_id').html();
            }else{
                return stateText;
            }
        };
