...
class CitizenProfileCrudController extends CrudController
{
    ...
    public function selectionSearch(Request $request){ // This is the function which I want to call from ajax
        ...
        if ($search_term)
        {
            $results = CitizenProfile::select(DB::raw('CONCAT(fName," ",mName," ",lName) as fullname, id, refID, suffix, address, bdate, brgyID, civilStatus, placeOfOrigin, purokID, sex, "CitizenProfile" as ownerType'))
                ->with('barangay', function ($q) use ($search_term) {
                    $q->orWhere('name', 'like', '%'.$search_term.'%');
                })
                ->orWhere('refID', 'like', '%'.$search_term.'%')
